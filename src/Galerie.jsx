...
const Galerie = () => {
  // Définir les chemins des images pour les femmes et les hommes
  const femmesImages = [
  ...
  const diviserEnGroupes = (images) => {
    const groupes = [];
    for (let i = 0; i < images.length; i += 6) {
      groupes.push(images.slice(i, i + 6));
    }
    return groupes;
  };

  // Obtenir les groupes d'images pour les femmes et les hommes
  const femmesGroupes = diviserEnGroupes(femmesImages);
  const hommesGroupes = diviserEnGroupes(hommesImages);
  ...
  return (
    <div className='galerie'>
      ...
      <div>
        ...
        <Carousel>
          {femmesGroupes.map((groupe, index) => (
      ...
      <div style={{ marginTop: '50px' }}>
        ...
        <Carousel>
          {hommesGroupes.map((groupe, index) => (
            <Carousel.Item key={index}>
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                {groupe.map((image, i) => (
